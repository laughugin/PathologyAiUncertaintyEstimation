...
def main():
    ...
    # Safe DataLoader worker count: CUDA + forked workers is a common failure mode.
    ...
    if nw_env is not None and str(nw_env).strip() != "":
        ...
        num_workers = min(4, int(cfg["data"].get("num_workers", 4)))
    pin_mem = device.type == "cuda" and num_workers == 0
    print(f"DataLoader num_workers={num_workers} pin_memory={pin_mem} device={device}")
