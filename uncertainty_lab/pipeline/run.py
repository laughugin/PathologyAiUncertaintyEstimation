...
def run_pipeline(
    ...
    *,
    ...
    logits_np, y_np = method.predict_logits(
        models, loader, device, mc_samples=mc_samples, on_batch=on_batch
    )

    report("Computing metrics…", 0.80)
    full_summary = compute_metrics_bundle(logits_np, y_np, eval_cfg, for_json=False)
    metrics_for_file = compute_metrics_bundle(logits_np, y_np, eval_cfg, for_json=True)
    ...
    if plot_hist:
        u = intr["uncertainty_one_minus_msp"]
        err = intr["error"]
        correct = 1 - err
